...
if (isset($_POST['submit'])) {
    ...
    if (empty($name) || empty($description) || empty($price) || empty($id_menus)) {
        ...
        if (empty($erreur)) {
            $stmt = $bdd->prepare("INSERT INTO plats (name, description, price, image_url, id_menus) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute(array($name, $description, $price, $image_url, $id_menus));
...
<head>
    ...
    <title>Ajouter un Plat - ArtResidence</title>
